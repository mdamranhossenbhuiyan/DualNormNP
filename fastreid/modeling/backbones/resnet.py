...
class  noise_injected_mixing(nn.Module):

    def __init__(self, num_features, alpha=0.1, eps=1e-6):
        """
        Args:
...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, with_nl, block, layers, non_layers):
        ...
        self.IN2 = nn.InstanceNorm2d(512) #+  self.beta.sample((B, 1,  1, 1)).to(x.device) #SpatialVariantMixStyle(), affine=True)
        self.IN3 = nn.InstanceNorm2d(1024) #+  self.beta.sample((B, 1,  1, 1)).to(x.device) #SpatialVariantMixStyle(), affine=True)
        self.random_init()
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layer1)):
            ...
            if i == self.NL_1_idx[NL1_counter]:
                ...
                NL1_counter += 1
        x = self.IN1(x) # +  self.beta1.sample((x.size(0), 1,  1, 1)).to(x.device)
  # layer 2
        NL2_counter = 0
        ...
        x = self.IN2(x) #+   self.beta2.sample((x.size(0), 1,  1, 1)).to(x.device)
        # layer 3
        NL3_counter = 0
        ...
        for i in range(len(self.layer3)):
            ...
            if i == self.NL_3_idx[NL3_counter]:
                ...
                NL3_counter += 1
        x = self.IN3(x) #+   self.beta3.sample((x.size(0), 1,  1, 1)).to(x.device)
